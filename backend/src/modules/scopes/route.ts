...
import { z } from "zod";
import { apiEnvelopeSchema, paginatedMetaSchema, scopeSchema } from "../../contracts/admin-api.js";
...
export const scopeRoutes: FastifyPluginAsync = async (app) => {
  app.get("/", {
    schema: {
      tags: ["scopes"],
      querystring: scopeListQuerySchema,
      response: {
        200: apiEnvelopeSchema(z.array(scopeSchema), paginatedMetaSchema)
      }
    },
  ...
  app.post("/", {
    schema: {
      ...
      response: {
        200: apiEnvelopeSchema(scopeSchema)
      }
    },
