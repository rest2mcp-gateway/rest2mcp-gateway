...
import { z } from "zod";
import { apiEnvelopeSchema, mcpServerSchema, paginatedMetaSchema } from "../../contracts/admin-api.js";
...
export const mcpServerRoutes: FastifyPluginAsync = async (app) => {
  app.get("/", {
    schema: {
      tags: ["mcp-servers"],
      querystring: mcpServerListQuerySchema,
      response: {
        200: apiEnvelopeSchema(z.array(mcpServerSchema), paginatedMetaSchema)
      }
    },
  ...
  app.post("/", {
    schema: {
      ...
      response: {
        200: apiEnvelopeSchema(mcpServerSchema)
      }
    },
