...
import { z } from "zod";
import { apiEnvelopeSchema, authUserSchema as contractAuthUserSchema, paginatedMetaSchema } from "../../contracts/admin-api.js";
...
export const userRoutes: FastifyPluginAsync = async (app) => {
  app.get("/", {
    schema: {
      ...
      response: {
        200: apiEnvelopeSchema(z.array(contractAuthUserSchema), paginatedMetaSchema)
      }
    },
  ...
  app.post("/", {
    schema: {
      ...
      response: {
        200: apiEnvelopeSchema(contractAuthUserSchema)
      }
    },
