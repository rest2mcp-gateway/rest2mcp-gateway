...
export const backendApiRoutes: FastifyPluginAsync = async (app) => {
  app.get("/", {
    schema: { tags: ["backend-apis"], querystring: backendApiListQuerySchema },
  ...
  app.post("/", {
    schema: { tags: ["backend-apis"], body: backendApiBodySchema },
  ...
  app.patch("/:id", {
    schema: { tags: ["backend-apis"], params: paramsSchema, body: backendApiUpdateSchema },
    preHandler: requireRoles(["super_admin", "admin", "editor"])
  }, async (request) => {
